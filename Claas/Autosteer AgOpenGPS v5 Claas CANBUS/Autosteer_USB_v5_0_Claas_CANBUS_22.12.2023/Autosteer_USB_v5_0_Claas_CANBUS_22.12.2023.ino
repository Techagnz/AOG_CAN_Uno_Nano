...
else if ((rxBuf[1])== 0 && (rxBuf[2])== 0) // stage5 models
{
  engageCAN = bitRead(rxBuf[0],2);
                Time = millis();
                digitalWrite(engageLED,HIGH); 
                relayTime = ((millis() + 1000));
...
else if ((rxBuf[0])== 39 && (rxBuf[2])== 241)  // MR models
{
  engageCAN = bitRead(rxBuf[1],0);
                Time = millis();
                digitalWrite(engageLED,HIGH); 
                relayTime = ((millis() + 1000));
                //*****Turn saftey valve ON**********
...
else if ((rxBuf[1])== 0 && (rxBuf[2])== 125)  // Pre MR models
{
  engageCAN = bitRead(rxBuf[0],2);
                Time = millis();
                digitalWrite(engageLED,HIGH); 
